...
import { getUserOrders, getOrderById, createReview, cancelOrder } from '../services/api';
import OrderTracker from '../components/OrderTracker';
import { FiClock, FiMapPin, FiStar, FiChevronRight, FiArrowLeft, FiHome, FiX } from 'react-icons/fi';
        ...
        <div className="container mx-auto px-4 py-6 max-w-3xl">
          ...
          <div className="bg-white rounded-2xl p-6 mb-4 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h1 className="text-xl font-bold mb-1">Order #{selectedOrder._id.slice(-8)}</h1>
          ...
          <div className="bg-white rounded-2xl p-4 mb-4 shadow-sm">
            <h3 className="font-bold text-lg mb-3">{selectedOrder.restaurant?.name}</h3>
            <div className="space-y-2 text-sm">
              <div className="flex items-start gap-2 text-gray-600">
                <FiMapPin className="mt-0.5 flex-shrink-0" size={16} />
                <p>
                  {selectedOrder.deliveryAddress?.street || 'N/A'}, {selectedOrder.deliveryAddress?.city || ''}, {selectedOrder.deliveryAddress?.state || ''} {selectedOrder.deliveryAddress?.zipCode || ''}
                </p>
              </div>
              <div className="flex items-center gap-2 text-gray-600">
                <FiClock size={16} />
                <p>{selectedOrder.estimatedDeliveryTime ? new Date(selectedOrder.estimatedDeliveryTime).toLocaleString() : 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          {/* Cancel Order */}
          {selectedOrder.status === 'placed' && canCancelOrder(selectedOrder.createdAt) && (
            <button
      ...
      <div className="container mx-auto px-4 py-6 max-w-3xl">
        {orders.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm mb-6">Start ordering delicious food!</p>
            <Link to="/" className="inline-block bg-orange-500 text-white px-8 py-3 rounded-xl font-medium hover:bg-orange-600">
              Explore Restaurants
            </Link>
          </div>
        ) : (
            ...
            {orders.map((order) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-bold text-base mb-1">{order.restaurant?.name}</h3>
